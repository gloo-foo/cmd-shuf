...
func TestShuf_DifferentSeedsDifferentOutput(t *testing.T) {
	...
	assertion.False(t, assertion.Condition(same), "different seeds should produce different output")
}
...
func TestShuf_RangeIgnoresStdin(t *testing.T) {
	...
	assertion.NoError(t, err)
	for _, l := range lines {
		assertion.True(t, assertion.Condition(l == "10" || l == "11" || l == "12"), "range output must come from 10..12, not stdin")
	}
	assertion.Count(t, lines, 3)
}
...
func TestShuf_EchoIgnoresStdin(t *testing.T) {
	...
	assertion.NoError(t, err)
	for _, l := range lines {
		assertion.True(t, assertion.Condition(l == "x" || l == "y"), "echo output must come from args, not stdin")
	}
