...
Behaviorally compatible with GNU `shuf` for the implemented flags. Because `shuf` writes a random permutation, output is never byte-identical to GNU `shuf`; compatibility is verified by the defining INVARIANTS (permutation, count, range, echo) in the `yupsh/bin-shuf` Docker integration harness against Debian coreutils.
